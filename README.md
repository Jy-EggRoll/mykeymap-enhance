...
>
> 本仓库的 `config.json` 旨在提供示例配置，事实上这就是我本人的 MyKeymap 配置。您可以备份您自己的配置文件，直接使用我的配置文件或者以文本形式复制特定片段，以简化您的配置。
>
...
- **用户行为识别**：仅当鼠标 **移动后再静置 200 ms 以上且移动范围超过 10 \* 10 px 时** 激活窗口，鼠标移动过程中绝不触发，鼠标一直静止时绝不触发。另外，当识别到存在用户从未访问过的窗口时（即从未将鼠标移动到该窗口上），自动激活窗口功能会自动停止，一旦用户访问过该窗口，自动激活就会恢复。
...
在 VSCode 较新的版本中，VSCode 自己会尝试接管窗口边框着色的功能，这可能导致冲突，从而出现颜色时不时失效、甚至显示黑线边框的问题。解决方法是将 VSCode 的 `window.border` 设为 `system`，这样就不会有任何问题。

该 VSCode 的新功能我认为是不合适、不稳定的，已经存在许多关于该功能的错误报告。我已发出反馈，请求其将默认设置改为 `system` 而非 `default`。
